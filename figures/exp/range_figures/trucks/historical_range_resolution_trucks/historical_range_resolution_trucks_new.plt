...
x_tic_base = 6
xtic_width = 12
dfile="historical_range_resolution_trucks_new.dat"
epsfile="historical_range_resolution_trucks_new.eps"
box_width = 3
height=1
...
set xrange [0:60]
set yrange [1:80]  noreverse nowriteback
...
set xtics font 'Calibri, 30' ( "4" x_tic_base,  "5" x_tic_base+xtic_width, "6" x_tic_base+2*xtic_width, "7" x_tic_base+3*xtic_width, "8" x_tic_base+4*xtic_width)
...
set xlabel offset 0, 0.5 "Resolution" font  'Calibri, 33''
...
plot dfile using (($1)-1)*alg_c+x_base:(($3))  title "Hytra-M"   with boxes fill pattern 5 lt 1 lw 0.5,\
     dfile using (($1)-1)*alg_c+x_base+box_width:(($4))  title "Hytra-NM" with boxes fill pattern 3 lt 1 lw 0.5,\
     dfile using (($1)-1)*alg_c+x_base+2*box_width:(($5))  title "R-tree" with boxes fill pattern 4 lt 1 lw 0.5